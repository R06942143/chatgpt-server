...
team_document = Chroma(collection_name="team_document",embedding_function=embeddings, client_settings=Settings(chroma_api_impl=os.getenv("CHROMA_API_IMPL"),
                                  chroma_server_host=os.getenv("CHROMA_SERVER_HOST"),
                                  chroma_server_http_port=8000))
team_conversation = Chroma(collection_name="team_conversation",embedding_function=embeddings, client_settings=Settings(chroma_api_impl=os.getenv("CHROMA_API_IMPL"),
                                  chroma_server_host=os.getenv("CHROMA_SERVER_HOST"),
                                  chroma_server_http_port=8000))
...
async def ask(text: str) -> Any:
    llm = OpenAI(openai_api_key=OPENAI_API_KEY, temperature=0.9) # 大的 temperature 会让输出有更多的随机性
    return llm(text)
...
async def add_chroma_documents(team_knowledge: TeamDocument) -> Any:
    ...
    team_document.add_texts(texts=[team_knowledge.text], metadatas=[{"source": team_knowledge.source}] ,ids=[document_id])
    return "ok"

@router.post("/team_conversation")
async def add_chroma_documents(team_document: TeamDocument1) -> Any:
    document_id = str(uuid4())
    team_conversation.add_texts(texts=[team_document.text[:3000]], metadatas=[{"source": team_document.source}] ,ids=[document_id])
    return "ok"

@router.get("/with-history")
async def ask_with_history(text: str) -> Any:
    chat_history = DynamoDBChatMessageHistory(table_name="conrad-conversation", session_id="123")
...
async def ask_with_history_and_google_search(text: str) -> Any:
    ...
    agent_chain = AgentExecutor.from_agent_and_tools(agent=agent, tools=tools, verbose=True, memory=memory＿234)
...
async def ask_with_embedding(history_embedding: HistoryAndEmbedding) -> Any:
    ...
    QUESTION_PROMPT = PromptTemplate(
        template=question_prompt_template, input_variables=["context", "question", "ans"]
    )
    ...
    result = chain({"input_documents": context, "question":question, "ans":ans}, return_only_outputs=True)
    
    team_conversation.add_texts(texts=["Human: " + ans + ",AI: " + result['output_text']], metadatas=[{"source": "chat_with_ai"}] ,ids=[document_id])
    ...
    template=team_prompt_template, input_variables=["question", "ans", "team_conversation"]
    )
    chain1 = LLMChain(llm=OpenAI(temperature=0.9), prompt=TEAM_PROMPT)
    result11 =chain1.run({"question": question, "ans": ans, "team_conversation":team_conversation_docs})
    # return result
    if "False" == (result11.replace("\n", "").lstrip()):
        return result['output_text']
    else:
        return "Suggestion:"+ result['output_text'] + "\nHere’s how other team members have responded to this question:" + result11
...
async def ask_with_embedding(
    ...
    overall_chain = SequentialChain(chains=[chain1, chain2, chain3], input_variables=["question", "team_document_docs"],output_variables=["urgent_key", "summarization", "response"], verbose=False)
    result = overall_chain({"question": question, "team_document_docs": team_document_docs})
    return {"urgency":"True" == (result["urgent_key"].replace("\n", "").lstrip()), "summarization":result["summarization"].replace("\n", "").lstrip(), "response": result["response"].replace("\n", "").lstrip()}
